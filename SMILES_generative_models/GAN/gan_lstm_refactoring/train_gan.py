...
from GAN.gan_lstm_refactoring.scripts.model import MolGen
...
# data = []
    ...
    # save model
    pi.dump(gan_mol, open(f'autotrain/train_GAN_121.pkl', 'wb'))
...
def auto_train(case,state,path_ds: str, lr: float = 0.0003, bs: int = 256, steps: int = 10, hidden: int = 256,fine_tune=False,feature_column=['Smiles']):
    ...
    # train model for 10000 steps
    if fine_tune:
        with open('GAN/gan_lstm_refactoring/weights/v4_gan_mol_124_0.0003_8k.pkl', "rb") as f:
            gan_mol = pi.load(f)
    state.gen_model_upd_status(case=case,model_weight_path=f'autotrain/GAN_weights/train_GAN_{case}',status=1)
    gan_mol.train_n_steps(loader, max_step=steps, evaluate_every=150)
 
    if not os.path.isdir(f'autotrain/GAN_weights/train_GAN_{case}'):
...
# def main(server_dir = 'train_dislip',
#          conditions : List[str] = ['ic50'],
#          case:str = 'Alzhmr',
#          data_path_with_conds = 'docked_data_for_train/data_5vfi.csv',
#          test_mode = False,
#          state = None,
...
#     Args:
#         server_dir (str, optional): Path to save weights. Defaults to 'train_cVAE_transformer_altz'.
...
if __name__ == '__main__':
    ...
    auto_train(case = "Alzheimer_regression",state=state,
                            ...
                            fine_tune=True,
                            path_ds='autotrain/data/data_mek.csv',
                        )
