...
def check_novelty_chembl(smiles: List[str],
        train_data_path: str = 'Sber_Alzheimer/docked_data_4j1r.csv',
...
if __name__=='__main__':
    ...
    # check_novelty_mol_path('GAN/CCDC_fine-tuning/data/database_CCDC.csv',["CS(=O)C1=CC(F)=CC(C2=CC=CC=C2)=C1","CC1=CC2=C(C=C1O)C(=O)C1=CC=CC=C12","O=C(NC1=CC=C([N+](=O)[O-])C=C1)C1=CC=C(C(F)(F)C2=CC=C(I)C=C2)C=C1","O=S(=O)(C1=CC=C(Cl)C=C1)C1=C(Cl)C=CC2=CC=CC=C12","N#CC1=CC([N+](=O)[O-])=CC([N+](=O)[O-])=C1[N+](=O)[O-]"],
    #               '0',
    #               'mol')
