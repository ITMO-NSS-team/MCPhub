...
def _resolve_existing_path(path_value: str | None) -> str | None:
    """Resolve path from state.json against container/project layout.

    Handles both variants stored in states:
    - `train_model_data/...`
    - `automl/train_model_data/...`
    """
...
def input_data_preparing(case:str,
    ...
    """
    state = TrainState()
    data_path = _resolve_existing_path(state(case,'ml')['data_path'])
...
#Example for train
    ...
    run_train_automl(case=task,path_to_save='ML_models',timeout=60*10)
